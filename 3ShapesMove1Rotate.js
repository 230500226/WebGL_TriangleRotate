...
function mainFunction(){
    ...
    void main() {
        ...
        vec2 rotatedPosition = vec2(
            vertexPosition.x * cosR - vertexPosition.y * sinR,
            vertexPosition.x * sinR + vertexPosition.y * cosR
        );
        gl_Position = vec4(uPosition + rotatedPosition, 0.0, 1.0);
    }`;
    ...
    const starAnimDirection = {x:(Math.random() * 2 - 1) * 0.0, y:(Math.random() * 2 - 1) * 0.6};
...
var rotationAngle = 0.0;
    function draw() {
        rotationAngle += 0.1;
        const uRotation = gl.getUniformLocation(programStar, 'uRotation');
